...
$sql3 = "SELECT borrowed_tbl.borrowed_id, borrowed_tbl.`return_date`, student_tbl.`number` FROM borrowed_tbl INNER JOIN student_tbl ON borrowed_tbl.`student_id` = student_tbl.`student_id`  WHERE borrowed_tbl.`smsStatus` = 0";
  ...
  while ($row = $result3->fetch_assoc()) {
    try {
      if ($dateToday == $row['return_date']) {
        ...
        $updateBorrowed =
          "update borrwoed_tbl set smsStatus = 1 ";
